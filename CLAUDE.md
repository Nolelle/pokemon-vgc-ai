...
- `data/usage/spreads.json` (consumed by `vgc.sets.load_usage_spreads`) is generated by
  ...
  auto-scraped). The checked-in file is still the **M-B** dump
  (`data/usage/gen9championsvgc2026regmb-1760.json`, 2026-07-15). Do not regenerate from
  M-C until a same-format chaos-stats file exists. Per id-normalized species, keeps the
  top 3 Stat Point spreads by usage share.
...
- **Hidden opponent spreads are a distribution, and it is not a confident one.**
  ...
  certainty). Those spreads have not been rebuilt for M-C.
